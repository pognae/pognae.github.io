...
특히 구글이 우대하는 E-E-A-T(경험, 전문성, 권위성, 신뢰성)를 완벽히 반영해야 합니다. 필자의 생생한 실제 '경험(Experience)'을 1인칭 시점의 체험담이나 일화 형식으로 본문 곳곳에 녹여내고, 공신력 있는 공식 기관의 지침이나 제도적 발표 자료를 인용하여 콘텐츠의 '신뢰성(Trustworthiness)'을 입증하십시오.`;
...
[출력 형식]
[TITLE]
여기에 핵심 키워드가 자연스럽게 들어가고 클릭률(CTR)을 높일 수 있는 매력적이고 자연스러운 제목을 60자 이내로 작성하세요. (마크다운 기호 없이 순수 텍스트로만 작성. 예: "~하는 방법", "주의해야 할 ~가지", "~의 장단점과 해결책")
...
4. 글 구조 (본론): 
   ...
   - 중요: 소제목이나 리스트 등 마크다운 문법을 사용할 때는, 위아래로 반드시 빈 줄(Enter)을 하나 이상 삽입하여 HTML 변환 시 오류가 발생하지 않도록 하세요.
   - 독창성(Originality)과 경험(Experience)을 입증하기 위해, 필자가 실제로 체험하거나 후기를 다룬 부분("제가 실제로 적용해 본 꿀팁", "이 방법으로 진행 시 주의했던 점")을 1인칭으로 서술하는 섹션을 반드시 한 섹션 이상 포함하세요.
   - 금융, 건강, 행정 등 신뢰성이 필요한 분야는 보건복지부, 국세청, 금융감독원 등 해당 분야의 신뢰할 수 있는 공식 행정기관 및 신뢰 기관의 지침을 직접 인용하고, 더 상세한 정보를 제공하는 공식 서비스명(예: 홈택스, 정부24 등)을 출처로 함께 안내하십시오.
