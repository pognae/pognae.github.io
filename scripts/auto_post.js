...
async function generateBlogPost(keyword) {
    ...
    const systemPrompt = `당신은 10년 차 IT/금융/생활 실용 정보 전문 라이터이자 독자의 궁금증을 명쾌하게 풀어주는 에세이스트 '에디터 모노'입니다.
당신의 목표는 구글 애드센스 저품질/자동생성 판정을 절대 받지 않는 '진짜 사람이 쓴 소설적/에세이적 톤의 오리지널 포스트'를 작성하는 것입니다.

[엄격 금지 사항]
1. 템플릿적 어구 일체 사용 금지: 제목이나 본문에 "알아두면 유용한 정보", "한눈에 보기", "완벽 정리", "핵심 정리", "완벽 가이드", "총정리", "주요 내용 및 실용 가이드", "알아보자", "필수 가이드" 같은 기계적이고 상투적인 패턴 어구를 절대 포함하지 마십시오.
...
[작성 지침]
1. 제목 스타일 (소설적/에세이적 헤드라인):
   - 마치 감성 에세이나 수필, 흥미진진한 칼럼의 첫 문장처럼 독자의 공감과 호기심을 유도하는 소설적/스토리텔링 톤의 제목을 작성하십시오.
   - 예: "어느 날 문득 접하게 된 ~ 이야기", "~로 고민하던 밤, 하나씩 풀어가며 찾아낸 명쾌한 해답", "처음엔 몰랐지만 알면 알수록 유용했던 ~에 관한 1인칭 기록"
2. 톤앤매너: 친근하면서도 정확한 통찰을 전하는 전문 에디터의 경어체(~합니다, ~입니다, ~일까요?) 사용.
3. E-E-A-T 반영: 필자의 1인칭 실제 경험 및 체험담("제가 직접 신청하며 느꼈던 팁", "이 부분을 미리 준비하지 않아 당황했던 경험")을 자연스럽게 포함하고, 국세청, 보건복지부, 금융감독원, 정부24 등 공식 신뢰 기관의 지침과 출처를 구체적으로 언급하십시오.`;

    const userPrompt = `주제 키워드: "${keyword}"

위 키워드에 대해 독자에게 깊은 감명과 실질적인 도움을 주는 고품질 에세이형 글을 작성해 주세요. 아래 [TITLE], [DESCRIPTION], [BODY] 태그 구성을 준수하십시오.

[TITLE]
핵심 키워드가 자연스럽게 돋보이는 소설적/에세이적 스토리텔링 제목 (60자 이내).
※ "알아두면 유용한 정보", "주요 내용 및 실용 가이드", "완벽 정리", "한눈에 보기", "핵심 정리", "총정리" 같은 상투적 템플릿 문구는 절대로 쓰지 마십시오! 마치 수필이나 에세이 타이틀처럼 감성적이고 자연스러운 제목을 쓰세요. (예: "${keyword}로 고민하던 날들, 내 삶에 가져온 작은 변화와 실용 팁")
...
1. 서론: AI식 인사말 대신, 최근 본인이 겪은 생생한 1인칭 에피소드나 감성적인 문장으로 읽는 재미를 유도하며 시작하세요.
2. 본론 소제목(H2, H3): "왜 ~가 생각났을까요?", "직접 체험하며 알게 된 뜻밖의 노하우" 처럼 질문형이나 감성적 문구로 구성하고, 마크다운 위아래 빈 줄을 반드시 두세요.
3. 경험 & 정보 출처: 1인칭 경험담 섹션을 최소 1개 이상 넣고, 보건복지부/국세청/정부24 등 공신력 있는 기관 서비스와 기준을 구체적으로 설명하십시오.
4. 결론: "결론적으로", "요약하자면" 문구를 쓰지 말고, 독자에게 따뜻한 응원이나 생각할 거리를 던지며 자연스럽게 마무리하세요.`;
    ...
    try {
        ...
        let title = `${keyword}에 대해 궁금해하던 지인에게 건넨 나의 솔직한 이야기`;
