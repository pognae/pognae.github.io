...
- **2026-07-23 (9차 - NIM 모델 롤백 및 글 없음 조치)**:
  - `meta/llama-3.3-70b-instruct` 모델 호출 시 발생하는 404 API 에러로 인해 신규 글 자동 생성이 완전히 막혀 빌드 결과물에 글이 누락되던 문제를 확인 및 조치했습니다.
  - 이전 검증 모델인 `meta/llama-3.1-70b-instruct`로 모델 설정을 롤백하여 API 호출 정상을 복원하고 깃허브에 반영했습니다.
